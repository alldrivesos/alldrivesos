...
import { ApiResponse, UserRequestDetails } from "../../lib/types/api";
...
export default function CompleteRequests() {
  ...
  const query = useQuery<ApiResponse<UserRequestDetails>>({
    queryKey: ["complete-requests", id],
    queryFn: async () => {
      let resp = await apiClient.get(
        `https://api.alldrivesos.com/api/service-request/user-fetch-details/${id}`,
      );
      return resp.data;
    },
  });
  useEffect(() => {
    ...
    if (query.data) {
      //@ts-ignore
      setServiceInfo(query.data);
    }
  }, [query.isLoading]);
  if (query.isLoading)
    return (
      <div className="min-h-screen grid place-content-center">
        <div className="flex flex-col w-full mx-4 max-w-lg">
          <h2 className="text-xl font-bold mb-4">Fetching Info</h2>
          <CurveLoader />
        </div>
      </div>
    );
  if (query.isError) {
    return (
      <div className="min-h-screen grid place-content-center">
        <div className="flex flex-col w-full mx-4 max-w-lg p-6 bg-white rounded-xl shadow-sm border border-red-300 text-red-700">
          <h2 className="text-xl font-bold mb-4">Error Loading Request</h2>
          <p>
            There was an error fetching the details for this request. Please try
            again later.
          </p>
        </div>
      </div>
  ...
  return (
    <div>
      <RequestForm driverId={query.data.data.driverQuote.id} />
    </div>
  );
}
export function RequestForm({ driverId }: { driverId: string }) {
