...
import { apiClient, getMyServices } from "../../../services/api/serviceApi";
...
export default function RequestListWithout({
  status,
  paymentStatus,
  action,
}: Props) {
  ...
  const columns = [
    ...
    columnHelper.accessor((row) => row.serviceName, {
      id: "Service Category",
      //@ts-ignore
      cell: (info) => <p className="fw-600">{info.row.original.serviceName}</p>,
